...
parser.add_argument('--tb_filename', default="", type=str)
parser.add_argument('--pretrained_path', default="", type=str)
parser.add_argument('--save_path', default="", type=str)
best_acc1 = 0

def getfactors(args,epoch):
    hf = args.hf
    N = args.epochs
    K = len(args.F)
...
def main_worker(gpu, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_top1,t_loss = train(train_loader, model, criterion, optimizer, epoch,args)
        tb.add_scalar('Train_Top1',t_loss,epoch)     
        tb.add_scalar('Train_loss',train_top1,epoch) 
        # evaluate on validation set
        acc1,v_loss = validate(val_loader, model, criterion, args)
        tb.add_scalar('Val_loss',v_loss,epoch)
        tb.add_scalar('Val_Top1',acc1,epoch) 
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.accloss:
           accloss = acc_func_deter(output, target)        
           alpha,beta=  getfactors(args,epoch)
           if i == 0:
              print("##########################################")
...
def save_checkpoint(state, is_best,epoch,args):
    filename = os.path.join(args.save_path,"checkpoint.pth")
    
    best = os.path.join(args.save_path,"best.pth")
    
    torch.save(state, filename)
    if epoch == 34 or epoch == 64 or epoch == 59 or epoch == 84 or epoch == 40 or epoch == 79:
        unique_filename = os.path.join(args.save_path,"checkpoint_"+str(epoch)+".pth")
